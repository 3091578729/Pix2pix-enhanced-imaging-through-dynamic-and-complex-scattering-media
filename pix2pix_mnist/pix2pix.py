...
def sample_images(batches_done):
    ...
    save_image(img_sample, "images/%s/edge_%s.png" % (opt.dataset_name, batches_done), nrow=5, normalize=True)
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        loss_dege = criterion_edge(fake_B,real_B)

        # Total loss
        loss_G = loss_GAN + lambda_pixel * loss_pixel #+ lambda_pixel * loss_dege
    ...
    if epoch % opt.sample_interval == 0:
        sample_images(epoch)
    writer.add_scalar("loss_G", total_loss_G / batch_num, epoch)
    writer.add_scalar("loss_D", total_loss_D / batch_num, epoch)
